...
<template>
  <div class="user-feed">
    ...
    <template v-else-if="result.items.length">
      <div class="history-list"><article v-for="brief in result.items" :key="brief.id" class="history-row"><div class="history-date"><strong>{{ brief.date.slice(8) }}</strong><span>{{ brief.date.slice(0, 7) }}</span></div><div class="history-content"><RouterLink :to="`/briefs/${brief.id}`" class="history-title">{{ brief.title }}</RouterLink><p class="meta"><span class="news-count"><Newspaper :size="14" :stroke-width="1.8" aria-hidden="true" />{{ brief.items.length }} 条新闻</span></p></div></article></div>
