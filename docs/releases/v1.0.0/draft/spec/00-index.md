...
产品版本 v1.0.0；当前阶段 ITERATE；原型修订 r2；来源基线、候选基线、批准基线均无。DISCOVER 成果提交为 `98fd673`，双端原型 r1 提交为 `2bfb26d`。本轮按用户截图调整用户端并拆分设计规范，保留现有业务；不冻结规范，也未开始正式应用实施。
...
4. [契约入口](../contracts/README.md)与[唯一 OpenAPI 主源](../contracts/openapi.json)：48 schemas、54 操作及 14 份 synthetic 固定样例。
...
当前可运行原型位于 `../prototype/`，包含 `user-web/`、`admin-web/` 两个 Vue 3 项目，共享语义组件与内存 adapter，分别加载各自主题和布局。新闻、运行、评估和发送结果均为模拟；没有 HTTP 后端、真实模型执行或邮件发送。两个开发端口各有独立 localStorage，可从页顶重置。
