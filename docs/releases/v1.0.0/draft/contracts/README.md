...
原型已有的枚举保持一致：来源为 `rss/newsnow/search`，其中 `search` 使用 Tavily 的 `web_search` 工具；阅读深度为 `精简/标准/深入`；模型角色为 `主模型/摘要模型/评估模型/子模型`。Source 的 `interval/upstreamInterval` 单位为**秒**，界面可换算为分钟。评估相关性/忠实度为 0–5，引用检查为 0–100%，费用为估算美元，延迟为秒。
...
| `savePreferences` | `PUT /me/preferences` | 提交当前 version，冲突返回 409，成功递增 |
| `saveDelivery` | `PUT /me/delivery-settings` | 忽略页面计算值，verified/nextRunAt 由后端决定 |
| `verifyEmail` | `POST /me/email-verification` 与 `/confirm` | 原型一键模拟验证；正式流程必须验证实际凭证 |
| `testDelivery` | `POST /me/delivery-test` | 验证收件地址；HTTP 202 不等于已送达 |
...
| `saveEvalCase/runEvaluation` | `POST/PUT /admin/eval-cases`、`POST /admin/evaluations` | 运行返回 evaluationId；固定样例实验不发邮件 |
| `toggleUser` | `PUT /admin/users/{id}/status` | 转显式 active/disabled，后端验证管理员 |

`SourceWrite/ModelWrite/ConfigWrite/DeliverySettingsWrite/EvalCaseWrite` 与响应 DTO 分离；adapter 只投递允许写入字段。连接测试返回能力验证结果，不能只凭 HTTP 200 标记支持全部工具。
...
新生成、投递重试、测试投递、手动采集和实验创建要求 `Idempotency-Key`；同身份/路由/key/请求体重放返回原操作，不生成重复工作。相同 key 搭配不同请求体返回 409。新一次用户意图使用新 key。SSE 的 event ID 在同一 run 内递增，`Last-Event-ID` 用于补拉；不是会话权限凭证。

## 冻结前仍须细化

当前契约刻意对齐可运行原型，尚不能代替完整后端规范：`RunEvent.time` 是页面时间文本，`DeliverySettings.nextRunAt` 是包含时区的展示文本；正式调度/事件将补充或替换为完整时间 DTO。Source 的未知时间为空串是原型兼容约定，持久层使用 null。
