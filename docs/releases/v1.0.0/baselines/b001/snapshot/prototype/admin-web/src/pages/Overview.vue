...
import { ArrowUpRight, Radio, Activity, Bell, ArrowRight } from 'lucide-vue-next';
...
<template>
  ...
  <div class="metric-grid">
    ...
    <div class="metric"><span class="meta">需要关注的投递 · 示例</span><strong>{{ failedDelivery.length }}</strong><span class="meta">站内通知失败或状态未知</span></div>
  ...
  <div class="admin-grid admin-section">
    <section class="card"><div class="section-heading"><h2>需要关注</h2><span class="meta">synthetic 演示事件</span></div>
      ...
      <ul v-else class="admin-list">
        ...
        <li v-for="delivery in failedDelivery.slice(0, 2)" :key="delivery.id"><div class="row"><Bell :size="18" class="admin-mini-icon" /><h3>{{ delivery.userName }}的站内通知</h3><Badge :status="delivery.status" /></div><p class="muted">{{ delivery.error || '站内通知发布状态待确认。' }}</p><RouterLink class="admin-subtle-link" to="/deliveries">查看投递记录 <ArrowRight :size="14" /></RouterLink></li>
      </ul>
    </section>
