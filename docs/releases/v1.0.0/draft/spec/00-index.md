...
产品版本 v1.0.0；当前阶段 ITERATE；原型修订 r1；来源基线、候选基线、批准基线均无。DISCOVER 成果已先提交至 Git（`98fd673`），随后按用户授权制作可运行原型，供下一轮调整页面和方案；本轮不冻结规范，也未开始正式应用实施。
...
当前可运行原型位于 `../prototype/`，包含 `user-web/`、`admin-web/` 两个 Vue 3 项目，共享设计样式、组件与内存 adapter。新闻、运行、评估和发送结果均为模拟；没有 HTTP 后端、真实模型执行或邮件发送。两个开发端口各有独立 localStorage，可从页顶重置。
