  ...
  <div class="app-shell--admin">
    ...
    <template v-if="route.path !== '/login'">
      <aside class="sidebar" aria-label="管理导航">
        <RouterLink class="brand" to="/overview" aria-label="知更管理概览"><BrandLogo decorative /><span class="brand-name">知更<span>ZHIGE NEWS</span></span></RouterLink>
      ...
      <div class="app-body">
        <header class="topbar"><div class="row"><button class="icon-button menu-toggle" aria-label="打开管理菜单" @click="menu = true"><Menu :size="20" /></button><RouterLink class="admin-mobile-brand" to="/overview" aria-label="知更管理概览"><BrandLogo :size="28" decorative /></RouterLink><span class="topbar-product">知更管理控制台</span><span class="breadcrumb-divider">/</span><strong class="topbar-title">{{ title }}</strong></div><span class="meta">Asia/Shanghai</span></header>
