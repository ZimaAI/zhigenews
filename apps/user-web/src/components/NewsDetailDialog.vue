  ...
  <Modal :open="open" :title="item.title" @close="$emit('close')">
    <div class="news-detail">
      ...
      <section v-if="item.citations.length" class="detail-citations" aria-label="关联来源">
        <h3>关联来源</h3>
        <ol><li v-for="citation in item.citations" :key="citation.id"><a :href="safeExternalUrl(citation.url)" target="_blank" rel="noopener noreferrer">{{ citation.title }}<ExternalLink :size="13" :stroke-width="1.8" aria-hidden="true" /><span class="sr-only">（新标签页）</span></a><p>{{ citation.name }} · {{ domain(citation.url) }}</p></li></ol>
      </section>
    </div>
  </Modal>
</template>
