...
<template>
  <div class="field">
    ...
    <ul v-if="modelValue.length" class="keyword-list user-plain-list"><li v-for="value in modelValue" :key="value" class="keyword-token"><span>{{ value }}</span><button type="button" :disabled="disabled" :aria-label="`移除关键词 ${value}`" @click="remove(value)"><X :size="14" aria-hidden="true" /></button></li></ul>
  </div>
</template>
<style scoped>
...
.keyword-token { display: inline-flex; align-items: center; gap: 4px; padding-left: 12px; border: 1px solid var(--color-border); border-radius: var(--radius-control); background: var(--color-surface-muted); font-size: 13px; font-weight: 400; min-width: 0; max-width: 100%; }
.keyword-token span { overflow-wrap: anywhere; }
.keyword-token button { display: flex; align-items: center; justify-content: center; border: 0; color: var(--color-text-secondary); background: none; width: 44px; min-width: 44px; height: 44px; border-radius: var(--radius-control); }
