...
import type { Source, SourceWrite, SourceBatchResult } from '@zhigenews/api-client';
...
const { items, loading, loaded, error: loadError, search, kind, selectedIds, page, total, totalPages, invalidTotal, allSelected } = collection;
...
async function save() {
  ...
  try {
    const source = await request<Source>(editId.value ? 'saveSource' : 'createSource', { path: editId.value ? { id: editId.value } : undefined, body: { ...form.value, name: form.value.name.trim(), sourceId: form.value.sourceId.trim(), url: form.value.url.trim() } });
    if (selected.value?.id === source.id) selected.value = source;
    editing.value = false; await collection.load(); notify('来源已保存。');
  } catch (error) { formError.value = errorText(error); } finally { busy.value = ''; }
}
async function act(source: Source, operation: 'fetchSource' | 'setSourceEnabled' | 'stopSource') {
  if (busy.value) return;
  busy.value = source.id; actionError.value = '';
  try {
...
async function batch(action: 'disable' | 'delete') {
  ...
  busy.value = 'batch'; actionError.value = ''; deleteError.value = '';
  try {
    const result = await request<SourceBatchResult>(allInvalid ? 'deleteInvalidSources' : 'batchSources', allInvalid ? {} : { body: { action, ids: action === 'delete' ? deleteIds.value : [...selectedIds.value] } });
    batchResult.value = result; batchLabel.value = action === 'delete' ? '删除' : '停用';
    collection.forget(result.succeeded); deleteDialog.value = null;
    await collection.load();
  } catch (error) {
    if (action === 'delete') deleteError.value = errorText(error); else actionError.value = errorText(error);
  } finally { busy.value = ''; }
}
...
useRefresh(() => { if (busy.value || editing.value || deleteDialog.value || loading.value) return; return route.params.id ? loadDetail() : collection.load(); }, 5000);
</script>
<template>
  <template v-if="route.params.id"><RouterLink class="admin-back" to="/sources"><ArrowLeft :size="16" />全部新闻来源</RouterLink><div v-if="detailLoading && !selected" class="skeleton" role="status" aria-label="加载来源详情"></div><EmptyState v-else-if="!selected" title="无法加载这个来源" :description="detailError || '来源不存在或已不可访问。'" error><button class="button" @click="loadDetail">重试</button></EmptyState>
    <template v-if="selected"><header class="page-header"><div><div class="eyebrow">SOURCE DETAIL</div><h1>{{ selected.name }}</h1><p>{{ selected.kind === 'newsnow' ? 'NewsNow' : 'RSS' }} · {{ selected.sourceId || selected.id }}</p></div><div class="actions"><span class="badge" :class="selected.health === 'invalid' ? 'badge--danger' : 'badge--primary'">{{ healthLabel(selected) }}</span><span class="badge">{{ enabled(selected) ? '已启用' : '已停用' }} {{ collectionLabel(selected) }}</span><button class="button" :disabled="!!busy" @click="edit(selected)"><Pencil :size="16" />编辑来源</button><button class="button button--primary" :disabled="!!busy || collecting(selected) || (!enabled(selected) && selected.health !== 'invalid')" @click="act(selected, 'fetchSource')"><RefreshCw :size="16" />{{ busy === selected.id ? '正在提交…' : '请求采集' }}</button></div></header>
      ...
      <section class="card"><h2>连接与快照</h2><dl class="admin-kv"><dt>请求地址</dt><dd><a :href="selected.url" target="_blank" rel="noopener noreferrer">{{ selected.url }}</a></dd><dt>来源标识</dt><dd class="mono">{{ selected.id }}</dd><dt>最近尝试</dt><dd>{{ formatDate(selected.lastFetchedAt) }}</dd><dt>最近成功</dt><dd>{{ formatDate(selected.lastSuccess) }}</dd><dt>下次采集</dt><dd>{{ formatDate(selected.nextFetch) }}</dd><dt>内容变化</dt><dd>{{ formatDate(selected.lastChanged) }}</dd><dt>快照取得时间</dt><dd>{{ formatDate(selected.snapshotFetchedAt) }}</dd><dt>快照 ID</dt><dd class="mono">{{ selected.snapshotId || '尚无快照' }}</dd><dt>上游配置版本</dt><dd class="mono">{{ selected.upstreamRevision || '未提供' }}</dd><dt>缓存状态</dt><dd>{{ selected.stale ? '待更新' : '有效' }}</dd></dl><button v-if="collecting(selected)" class="button button--danger" :disabled="!!busy || selected.collectionStatus === 'stopping'" @click="act(selected, 'stopSource')">{{ selected.collectionStatus === 'stopping' ? '停止中…' : '停止采集并停用' }}</button><p class="admin-note">条件请求未发现变化时继续使用原快照；最近成功时间与快照内容取得时间分别记录。</p><button class="button" :disabled="!!busy" @click="act(selected, 'setSourceEnabled')"><component :is="!enabled(selected) ? Play : Pause" :size="16" />{{ !enabled(selected) ? '启用来源' : '停用来源' }}</button></section>
    </template>
  </template>

  <template v-else>
    <header class="page-header"><div><div class="eyebrow">NEWS SOURCES</div><h1>新闻来源</h1><p>按名称搜索全部来源，管理采集与失效状态。</p></div><div class="actions"><button class="button" :disabled="loading || !!busy" @click="collection.load()"><RefreshCw :size="16" />刷新</button><button class="button button--primary" @click="edit()"><Plus :size="16" />添加来源</button></div></header>
    ...
    <form class="admin-toolbar" @submit.prevent="pageError = ''; collection.applyFilters()">
      ...
      <label class="field">来源类型<select v-model="kind" class="field-control" :disabled="!!busy" @change="pageError = ''; collection.applyFilters()"><option value="all">全部类型</option><option value="newsnow">NewsNow</option><option value="rss">RSS</option></select></label>
      <button class="button button--primary" type="submit" :disabled="loading || !!busy">搜索</button>
    </form>
    <div class="actions source-batch"><span aria-live="polite">已选 {{ selectedIds.size }} 条</span><button class="button" :disabled="!selectedIds.size || !!busy || loading" @click="batch('disable')">停用所选来源</button><button class="button button--danger" :disabled="!selectedIds.size || !!busy || loading" @click="askDelete('selected')">删除所选来源</button><button class="button button--danger" :disabled="!loaded || !invalidTotal || !!busy || loading" @click="askDelete('invalid')">删除全部失效来源（{{ invalidTotal }}）</button><button v-if="selectedIds.size" class="button button--ghost" :disabled="!!busy" @click="selectedIds = new Set()">清空选择</button></div>
    <p class="admin-note">翻页保留选择；更换搜索或类型筛选会清空选择。“全部失效来源”覆盖数据库全部来源，包含已停用项。</p>
    <p v-if="loading && loaded" role="status" class="meta">正在更新来源列表…</p>
    ...
    <div v-else class="table-wrap" tabindex="0" role="region" aria-label="新闻来源列表，可横向滚动" :aria-busy="loading"><table class="data-table admin-table source-table"><thead><tr><th><label class="source-check"><input type="checkbox" :checked="allSelected" :indeterminate="!allSelected && items.some(item => selectedIds.has(item.id))" :disabled="loading || !!busy" @change="collection.togglePage()"><span>本页</span></label></th><th>来源</th><th>健康 / 采集状态</th><th>采集周期</th><th>最近成功 / 下次采集</th><th>操作</th></tr></thead><tbody>
      <tr v-for="source in items" :key="source.id"><td><label class="source-check"><input type="checkbox" :aria-label="`选择 ${source.name}`" :checked="selectedIds.has(source.id)" :disabled="loading || !!busy" @change="collection.toggle(source.id)"></label></td><td class="wide-cell"><RouterLink :to="`/sources/${source.id}`">{{ source.name }}</RouterLink><small>{{ source.kind === 'rss' ? 'RSS' : 'NewsNow' }}</small><small>{{ source.sourceId || source.url }}</small><small>{{ enabled(source) ? '已启用' : '已停用' }}</small></td>
      ...
      <td>{{ source.interval }} 秒<small>上游 {{ source.upstreamInterval ? `${source.upstreamInterval} 秒` : '未提供' }}</small></td><td>{{ formatDate(source.lastSuccess) }}<small>{{ formatDate(source.nextFetch) }}</small></td>
      <td><div class="actions"><button class="button button--small" :disabled="!!busy || collecting(source)" @click="edit(source)">编辑</button><button class="button button--small" :disabled="!!busy || collecting(source) || (!enabled(source) && source.health !== 'invalid')" @click="act(source, 'fetchSource')">{{ source.health === 'invalid' ? '手动重试' : '采集' }}</button><button class="button button--small" :disabled="!!busy || source.collectionStatus === 'stopping'" @click="act(source, 'setSourceEnabled')">{{ enabled(source) ? '停用' : '启用' }}</button><button v-if="collecting(source)" class="button button--small button--danger" :disabled="!!busy || source.collectionStatus === 'stopping'" @click="act(source, 'stopSource')">{{ source.collectionStatus === 'stopping' ? '停止中…' : '停止采集' }}</button></div><small v-if="source.error">{{ source.error }}</small></td></tr>
    </tbody></table></div>
