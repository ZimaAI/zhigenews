...
用户截图标记来源前的字母方块，要求移除并恢复更新按钮旧图标。历史核对 r2/72b512a 与 r1/2bfb26d 为 WandSparkles 16px，r3/e7251f3 为 RefreshCw。采用原设计的魔杖星光图标，保留本轮统一描边、按钮文字与生成状态；NewsArticle 移除来源字母节点及无用 CSS，来源名称、时间和话题标签保留。用户设计规范和原型/UX/变更文档同步。

npm run check 通过；浏览器今日页与详情页 source-letter 节点均为 0，来源名称仍可见，更新按钮为 lucide-wand-sparkles-icon。实际 670px 页宽无横向溢出，截图目视确认，控制台 error 为空；留在 /today。只针对两处视觉调整检查，未重复上轮完整交互和 18 组布局检查，也未修改领域/契约/管理员。证据：evidence/prototype-r4-icons-validation.json。

继续 ITERATE r4，无阶段推进问题。最新检查点目标 c0006，以 state 为准；main/e7251f3 与上一轮未提交修改均保留。
