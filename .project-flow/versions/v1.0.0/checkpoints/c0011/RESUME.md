...
阶段提示：FRONTEND_VERIFY；检查点：c0011。状态主源为 state.json，本文是导航。
...
下一步提示：前后端实现已完成；由用户按IMPLEMENTATION.md配置真实模型/Tavily并进行最终业务验收，记录结果与问题后续修复；保留FRONTEND_VERIFY，不把未执行检查视为通过
