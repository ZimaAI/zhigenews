...
模型密钥由环境主密钥加密，API只写；主密钥丢失后无法解密，生产需妥善保存。CLI环境初始模型为未验证、配置为草稿，真实能力测试后才能发布。真实模型/Tavily/LLM评分仍未验证；用户本轮明确接受当前后端交接并授权前端，外部验证移至完整实现后的用户验收。
...
代码身份由backend-verification.json的source_fingerprint绑定backend、compose.yaml、infra。新会话运行`source-id --component backend --frozen`核对当前源码，重新build并启动后实际探测，不能仅凭旧容器health声明代码一致。此目录仍为候选，实际hNNN以state.json为准。
