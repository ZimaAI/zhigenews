...
<template>
  <div class="user-reading">
    ...
    <template v-else>
      <header class="detail-header"><p class="meta">{{ brief.date }} · {{ brief.items.length }} 条新闻</p><h1>{{ brief.title }}</h1><p class="detail-summary">{{ brief.summary }}</p><RouterLink class="user-inline-link" :to="`/runs/${brief.runId}`">整理记录<ArrowUpRight :size="14" aria-hidden="true" /></RouterLink></header>
...
.detail-header .user-inline-link { min-height: 44px; }
