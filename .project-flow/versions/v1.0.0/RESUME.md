# v1.0.0 新会话续接入口

阶段提示：DISCOVER；检查点：c0001。状态主源为 state.json，本文是导航。

```text
$prototype-to-product
继续当前工作区的 v1.0.0。先运行 resume 检查并读取当前阶段资料；
沿用仍有效的已批准决定，只推进已授权范围，不重新从零规划。
若文件或代码已变化，先核对差异，不恢复旧文件覆盖现状。
```

下一步提示：等待用户对q0001选择进入ITERATE或修改DISCOVER；进入后制作双端Vue原型并同步领域/契约/验收。

新会话必须检查实际工作目录、未提交修改、环境、服务与契约。
旧日志、端口号和进程 PID 不代表当前服务可用；不要复制密钥到交接资料。
会话切换不等于新产品版本、新基线或新的构建授权。
SPECIFY 中继续整理不等于批准；READY 中继续查看不等于授权后端。
下一阶段的具体说法见技能包 USAGE.zh-CN.md；基线号以当前 state 为准。

## 阶段确认点

q0001 / awaiting_user

当前 DISCOVER 已具备收尾条件。要进入 ITERATE（开始 ITERATE 的工作；仍遵守原范围与授权），还是继续修改当前阶段的内容？也可以暂时停在这里。

先核对当前内容与已记录回答，不把换会话视为同意。
