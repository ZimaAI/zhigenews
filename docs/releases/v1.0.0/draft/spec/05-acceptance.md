...
| --- | --- | --- | --- |
| AC-UI-001 | REQ-PLT-001, REQ-UX-001 | frontend | 用户/管理员为两个可独立启动与构建的 Vue 3 项目；共用设计变量和契约；首页持续标明模拟数据，重置恢复固定时钟与 seed。 |
| AC-UI-002 | REQ-AUTH-001, REQ-UX-001 | frontend | 登录/注册有必填与格式校验、提交中、失败保留输入和退出；普通用户进入管理入口显示无权限；原型不宣称真实鉴权。 |
| AC-UI-003 | REQ-PREF-001 | frontend | 初次设置可添加/移除主题、包含与排除关键词、切换 prefer/required、来源/语言/窗口/条数/深度；保存后显示新版本；失败不丢编辑值。 |
